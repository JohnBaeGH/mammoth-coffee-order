...
def add_order():
    ...
    if name in session['orders']:
        ...
        session['drink_counts'][(old_drink, old_temp)] -= 1
        if session['drink_counts'][(old_drink, old_temp)] == 0:
            del session['drink_counts'][(old_drink, old_temp)]
    
    # 새 주문 추가
    session['orders'][name] = (drink, temperature, request_note)
    session['drink_counts'][(drink, temperature)] += 1
