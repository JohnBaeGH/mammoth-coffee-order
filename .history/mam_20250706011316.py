...
class CoffeeOrderApp:
    ...
    def create_widgets(self):
        ...
        # 주문 버튼
        self.add_order_btn = ttk.Button(menu_card, text="내 주문 완료", command=self.add_order, style='Accent.TButton', state='normal')
        self.add_order_btn.pack(fill=tk.X, pady=(8, 0))
        ...
        status_card.pack(fill=tk.BOTH, expand=True, pady=8)
        
        ttk.Label(status_card, text="현재 주문 현황", style='Header.TLabel').pack(anchor='w')
        self.status_text = tk.Text(status_card, height=6, width=40, font=self.FONT_BODY, 
                                  bg=self.COLOR_BACKGROUND, fg=self.COLOR_TEXT, 
        ...
        ttk.Button(button_frame, text="모든 인원 주문 완료", command=self.show_final_order, style='Accent.TButton').grid(row=0, column=0, sticky='ew', padx=(0, 5))
