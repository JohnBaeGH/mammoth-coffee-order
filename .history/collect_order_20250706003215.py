...
class CoffeeOrderApp:
    ...
    def setup_ui(self):
        ...
        # 상단에 매머드커피 매장 사진 표시
        try:
            main_img = Image.open(os.path.join("images", "mammoth_main.png")).resize((700, 180))
            self.main_photo = ImageTk.PhotoImage(main_img)
            img_label = tk.Label(main_frame, image=self.main_photo, borderwidth=0)
            img_label.grid(row=0, column=0, columnspan=3, pady=(0, 10))
        except Exception as e:
            img_label = tk.Label(main_frame, text="MAMMOTH COFFEE", font=("Arial", 32, "bold"), fg="#333")
            img_label.grid(row=0, column=0, columnspan=3, pady=(0, 10))

        # 타이틀 및 안내문구
        title_label = ttk.Label(main_frame, text="☕ 매머드커피 주문 시스템 ☕", font=('Arial', 20, 'bold'), foreground="#222")
        title_label.grid(row=1, column=0, columnspan=3, pady=(0, 5))
        subtitle_label = ttk.Label(main_frame, text="아래에서 음료를 선택하고 주문을 진행하세요!", font=('Arial', 12), foreground="#666")
        subtitle_label.grid(row=2, column=0, columnspan=3, pady=(0, 15))
        ...
        name_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        self.menu_frame.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        # 주문 버튼
        order_btn = ttk.Button(self.menu_frame, text="내주문 완료", command=self.add_order)
        order_btn.grid(row=2, column=0, columnspan=2, pady=(10, 0))
        
        # 메뉴 이미지 선택 프레임
        self.image_frame = ttk.LabelFrame(main_frame, text="메뉴 선택 (이미지)", padding="10")
        self.image_frame.grid(row=5, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        self.status_frame.grid(row=6, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        ttk.Button(button_frame, text="모든 인원 주문 완료", command=self.show_final_order).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(button_frame, text="초기화", command=self.reset_orders).pack(side=tk.LEFT)
